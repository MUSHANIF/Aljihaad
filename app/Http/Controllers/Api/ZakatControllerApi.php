...
{  
    ...
    public function amilZakatAbsen(Request $request){
        ...
        $cekPengurusNonStatus = amil_zakat::with(['getPengurusData'])
        ->where('id_pengurus', $request->id_pengurus)
        ->where('waktu_hadir', $request->waktu_berzakat)
        ->where('tanggal_hadir', $request->tanggal)
        ->first();
