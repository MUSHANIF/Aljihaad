...
export default function UpdatePembagianZakat({
  auth,
  Total_beras,
  jumlah_zakat,
  ...
  const { data, setData, post, errors, reset } = useForm({
    alamat: "",
    telepon: "",
    pilihan: getDataZakat?.jenis_pemilihan || "",
    jumlah_beras: getDataZakat?.jumlah_beras || 0,
    uang: getDataZakat?.jumlah_uang || 0,
    jenis_pengambilan: getDataZakat?.jenis_pengambilan || "",
    id_rt: getDataZakat?.id_rt,
  ...
  useEffect(() => {
    ...
    DataPengambilan.map((item) => {
      if (item.id === getDataZakat.jenis_pengambilan) {
        setJenisPengambilan(item);
      }
    });
  }, [data.pilihan]);
  return (
              ...
              <div className="mx-auto flex justify-center">
                <SelectButton
                  ...
                  options={[
                    ...
                    { label: "Amil", value: 4 },
                  ].filter((option) => option.value == data.pilihan)}
                />
              </div>
